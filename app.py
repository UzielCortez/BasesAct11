...
def add():
    ...
    user_id = session["user_id"]
    try:
        supabase.table("tasks").insert({"title": title, "done": False, "user_id": user_id}).execute()
...
def done(task_id):
    ...
    user_id = session["user_id"]
    try:
        supabase.table("tasks").update({"done": True}).eq("id", task_id).eq("user_id", user_id).execute()
